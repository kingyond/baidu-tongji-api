...
class RsaPublicEncrypt {
     ...
     * @param string $path
     */
    public function __construct($puk, $path)
    {
        $this->error = '';
        if(empty($puk) && (empty($path) || !is_dir($path)))
        {
            $this->error = "[error] error public key path: {$path}" . PHP_EOL;
        }
        $this->puk = $puk;
        $this->path = $path;
    }
    ...
    {
        if (!empty($this->puk)) {
            $this->publicKey = openssl_pkey_get_public($this->puk);
            return true;
        }
        ...
        $this->puk = file_get_contents($file);
        $this->publicKey = openssl_pkey_get_public($this->puk);
        return true;
    }
